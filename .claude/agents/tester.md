...
## Playwright MCP — 브라우저 UI 검증

프론트엔드 UI 변경이 포함된 경우, Vitest 외에 **Playwright MCP를 사용하여 실제 브라우저에서 검증**한다.

1. dev 서버 실행: `cd ui && npx next dev --port 3099 &` (백그라운드)
2. `mcp__playwright__browser_navigate` → 각 페이지 접속
3. `mcp__playwright__browser_snapshot` → DOM 구조/텍스트 검증
4. `mcp__playwright__browser_click` → 네비게이션/인터랙션 테스트
5. `mcp__playwright__browser_console_messages` → 콘솔 에러 0건 확인
6. 테스트 완료 후 dev 서버 종료

**필수 검증 항목**:
- 모든 페이지 렌더링 정상 (빈 화면/에러 없음)
- 네비게이션 동작 (링크 클릭 → 올바른 페이지)
- API 데이터가 UI에 반영됨
- 콘솔 에러 0건
...
- [ ] Playwright MCP로 브라우저 UI 검증 완료 (프론트엔드인 경우)
