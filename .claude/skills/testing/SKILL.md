...
## Playwright MCP — 브라우저 기반 UI 검증

프론트엔드 UI 프로젝트에서는 Playwright MCP 도구를 사용하여 **실제 브라우저에서 UI를 검증**한다.
Vitest 단위 테스트와 병행하여, 렌더링/네비게이션/인터랙션을 실제로 확인한다.

### 사용 가능한 도구

| 도구 | 용도 |
|------|------|
| `mcp__playwright__browser_navigate` | URL 접속 |
| `mcp__playwright__browser_snapshot` | 접근성 스냅샷 (DOM 구조 확인) |
...
| `mcp__playwright__browser_wait_for` | 요소/네트워크 대기 |
...
2. browser_navigate → http://localhost:3099
3. browser_snapshot → DOM 구조 확인 (요소 존재, 텍스트 내용)
4. browser_click → 네비게이션/인터랙션 검증
5. browser_snapshot → 변경 후 상태 확인
6. browser_console_messages → 콘솔 에러 0건 확인
7. dev 서버 종료
```

### 검증 체크리스트 (프론트엔드)

- [ ] 모든 페이지 접속 시 렌더링 정상 (빈 화면/에러 없음)
- [ ] 네비게이션 링크/탭 클릭 시 올바른 페이지로 이동
- [ ] API 데이터가 UI에 정확히 반영됨
- [ ] 콘솔 에러 0건 (404, TypeError 등)
- [ ] 인터랙티브 요소 (아코디언, 드로어, 필터 등) 정상 동작

### 주의사항

- `browser_snapshot`이 `browser_take_screenshot`보다 우선 — 스냅샷으로 DOM 구조와 텍스트를 정확히 검증
- 스크린샷은 시각적 확인이 필요할 때만 사용
- dev 서버는 테스트 시작 시 실행하고, 완료 후 반드시 종료
- 스냅샷의 `ref` 값을 사용하여 `browser_click`으로 정확한 요소 클릭
