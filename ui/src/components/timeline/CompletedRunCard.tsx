...
export function CompletedRunCard({
  runId,
  intentDescription,
  nodesTotal,
  nodesCompleted,
  createdAt,
  completedAt,
}: CompletedRunCardProps) {
  return (
    <Card
      className="hover:border-slate-300 transition-colors"
      data-testid="completed-run-card"
    >
      <div className="flex items-center justify-between p-4 gap-4">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <div className="w-8 h-8 rounded-full bg-emerald-50 flex items-center justify-center shrink-0">
            <span className="material-symbols-outlined text-emerald-500 text-base filled">
              check_circle
            </span>
          </div>
