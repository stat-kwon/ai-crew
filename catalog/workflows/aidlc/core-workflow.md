...
- After Inception completes, run `/crew:run` to start the Graph Executor
- Units Generation results are mapped to `.ai-crew/graph.yaml`
- Each unit runs as a separate agent (worktree-isolated) for parallel code generation
